        ...
        <header className="v3fs-hero">
          ...
          {days != null ? (
            <p className="v3fs-hero-line">
              <b>{days}</b> day{Math.abs(days) === 1 ? "" : "s"} {days >= 0 ? "to the first stakeholder demonstration" : "past the planned first demonstration"}
            </p>
          ) : null}
          {view === "flow" ? (
            // The canvas view carries the working context; every other view
            // stays quiet — one line of hero, then the content.
            <>
    ...
    <div className="v3fs-today">
      ...
      {open.length === 0 && inbox.length === 0 ? (
        <div className="v3fs-quiet">
          ...
          <p>ATOS keeps working between conversations — anything consequential arrives here first, and the full record stays one tap below.</p>
        ...
        <section className="v3fs-inbox" aria-label="Waiting on you">
          <div className="v3fs-ph">
            ...
            <span>{open.length + inbox.length} item{open.length + inbox.length === 1 ? "" : "s"} — decisions first, then evidence in quarantine</span>
          </div>
          {open.map((decision) => (
