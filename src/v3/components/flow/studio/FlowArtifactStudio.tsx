                ...
                <details className="v3fs-disc v3fs-disc-sm v3fs-dv-colophon">
                  <summary>
                    ...
                    <span className="v3fs-disc-hint">
                      {[draft && typeof draft.generatedAt === "string" ? `generated ${String(draft.generatedAt).slice(0, 10)}` : null,
                        ...
                        // Beside the confidence figure ON PURPOSE. A single-draft
                        // ontology carries a confidence it did not earn, and the
                        // two facts are only useful together.
                        artifact.provenanceWarning,
                  ...
                  <div className="v3fs-disc-b">
                    <div className="v3fs-dv-facts">
                      ...
                      {/* HOW IT WAS MADE. The document's own words, not a
                          re-derivation: the note says whether anything in here
                          cleared an agreement bar, and whether re-running is
                          the fix. Only drawn where a document carries the
                          stamp, so nothing older sprouts an empty row. */}
                      {draft && typeof draft.reconciliation === "object" && draft.reconciliation !== null
                        && typeof (draft.reconciliation as Record<string, unknown>).note === "string" ? (
                        <div className="v3fs-dv-fact">
                          <span className="v3fs-dv-fl">How it was made</span>
                          <span className="v3fs-dv-fv">
                            {String((draft.reconciliation as Record<string, unknown>).note)}
                          </span>
                        </div>
                      ) : null}
                      {edited ? (
