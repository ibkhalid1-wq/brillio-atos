  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch(`${FUNCTIONS_BASE}/flow-portal`, {
        ...
        body: JSON.stringify({ token, answers: composed }),
      });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(typeof body.error === "string" ? body.error : "Could not send your answers.");
      setState({ phase: "sent" });
    } catch (err) {
          ...
          {state.phase === "loading" ? (
            ...
            <>
              ...
              <div className="v3fs-portal-qs">
                ...
                <button type="button" className="v3fs-btn pri v3fs-portal-send" disabled={submitting || composed.trim().length < 20} onClick={() => void submit()}>
