  ...
  return (
    <AdamCard>
      <AdamCardBody className="v3-program-detail-stack" padded>
        ...
        {health && program.healthHeatmapGeneratedAt ? (
 ...
 * risk overlay) and the user's validation/de-risking stages. The critical-path
...
export default function RoadmapView({
  ...
  // The Gantt shows each phase's composite gate/readiness score — the same number
  // surfaced elsewhere in the app (e.g. Strategy 97%) — so the timeline progress
  // agrees with the gate rather than a separate artifact-completion tally.
  const gateScores = React.useMemo(() => {
    const map = new Map<string, number>();
    if (program) {
      for (const p of program.phases) {
        map.set(p.id, computePhaseReadiness(program, p.id).score);
      }
    }
    return map;
  }, [program]);
