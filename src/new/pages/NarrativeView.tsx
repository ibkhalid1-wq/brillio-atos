            ...
            <div style={{ position: "relative" }}>
              ...
              {showExportMenu && narrative && (
                <div style={{ position: "absolute", right: 0, top: "100%", marginTop: 4, background: "var(--v3-surface)", border: "1px solid var(--v3-border)", borderRadius: 6, boxShadow: "0 4px 12px rgba(0,0,0,0.12)", zIndex: 50, minWidth: 130 }}>
                  {([["PDF", () => { exportAsPDF(`Programme Narrative — ${program?.name || ""}`, narrativeToHtml({ name: program?.name, client: (program as Record<string, unknown>).client as string | undefined }, narrative, confidence ?? 0, program?.narrativeGeneratedAt || new Date().toISOString())); setShowExportMenu(false); }], ["Word Doc", () => { exportAsDocx(`Programme Narrative — ${program?.name || ""}`, narrativeToHtml({ name: program?.name, client: (program as Record<string, unknown>).client as string | undefined }, narrative, confidence ?? 0, program?.narrativeGeneratedAt || new Date().toISOString())); setShowExportMenu(false); }]] as Array<[string, () => void]>).map(([label, fn]) => (
