...
export const ADAM_USER_GUIDE: GuideArticle[] = [
  {
    id: "gs-001",
    title: "What ADAM helps you do",
    category: "Getting Started",
    keywords: ["what is adam", "about adam", "overview", "methodology", "what does adam do", "introduction"],
    summary: "ADAM helps teams move from a business problem to a production-ready Human + Agent capability with clear phase gates, stronger evidence, and visible accountability.",
    body: `
# What is ADAM?

ADAM stands for **AI-Driven Acceleration Methodology**. It is a structured framework that guides your organization through every stage of an AI transformation program - from defining the business problem to measuring delivered value.
...
ADAM enforces phase sequencing. You must complete Strategy before Mobilise, and so on. The Copilot will tell you exactly what is needed at each gate.

## Tips
- You do not need to complete a phase in one sitting - save as you go
- Upload documents at any phase to let ADAM extract relevant inputs automatically
- Use the Readiness Score at the top of your program to track overall health
  ...
  },
  {
    ...
    category: "Getting Started",
    keywords: ["readiness score", "adam readiness index", "score", "percentage", "health", "progress", "how is score calculated"],
    summary: "The ADAM Readiness Index combines evidence quality, decisions, approvals, stakeholder coverage, and freshness into one score that shows how ready the program is to progress.",
    body: `
# Understanding the Readiness Score

The **ADAM Readiness Index** is a live health indicator for your program. It tells you - at a glance - how ready your program is to advance.
...
- Resolve any blockers shown in the phase gate panel
  ...
  },
  {
    ...
    summary: "Upload decks, Word files, or PDFs and ADAM will extract likely inputs, stage proposals, and preserve your manual edits for review.",
    body: `
# Uploading Documents

ADAM can read your existing documents - strategy decks, business cases, architecture specs, project plans - and automatically extract relevant inputs to populate your program.
...
4. ADAM will parse the document and show you what it found

## What ADAM extracts by phase
...
- ADAM shows you a **review panel** with each extracted item and a confidence score
...
- ADAM never overwrites content you have manually edited - it creates **proposals** instead
- If you upload a newer version of the same document, ADAM detects the update and flags any decisions that may be affected
...
## Exit criteria
Before advancing to Mobilise, ADAM requires:
...
Upload a CSV or Excel file with columns: Role, Name, Allocation. ADAM will match each row to a delivery role and rebuild the roster cleanly.

### Required roles for AI programs
If your program includes L3 or L4 autonomous capabilities, ADAM requires:
...
Capability DNA is ADAM's framework for defining AI capabilities with precision. Each capability card captures:
...
ADAM enforces that the person who creates an artifact cannot be the same person who approves it. If you are working alone, assign a reviewer from the steering committee.
...
For each KPI defined in Discover, record the actual measured value. ADAM will automatically calculate:
...
These feed ADAM's global learning loop and will surface as insights for future programs in the same industry.
...
The ADAM Copilot is not a generic chatbot. It knows your program, your current phase, and exactly where your gaps are.
...
ADAM never silently overwrites content you have written. When the Copilot generates an update to a manually-edited artifact, it creates a **proposal** for you to review.

## How proposals work

1. You manually edit an artifact (for example the Outcome Charter)
2. You upload a new document or change a phase input
3. ADAM detects that the artifact could be improved based on the new information
4. Instead of overwriting, ADAM creates a proposal showing:
   - **Current content** - what you wrote
   - **Proposed content** - what ADAM suggests
   - **Reason** - why ADAM is suggesting the change
...
Artifacts are the formal outputs of each ADAM phase. They are the evidence that a phase has been completed to the required standard.
...
Some artifacts (for example the Capability DNA sign-off) are **signature artifacts** - they require a formal named approval before they unlock downstream phases.
  ...
  },
  {
    ...
    summary: "ADAM keeps both program-level and artifact-level history so you can recover from mistakes without losing the wider program state.",
    body: `
# Undo and Version History

ADAM protects your work with two levels of undo:

## Program-level undo (20 snapshots)
Every time you make a significant change to your program, ADAM saves a snapshot. You can step back through the last 20 states.
...
- Deletions that have been confirmed twice (ADAM asks for double confirmation on destructive actions)
...
If you are about to make significant changes to a complex artifact, use **Export Program** first to create a portable backup.
  ...
  },
  {
    ...
    summary: "When a gate is blocked, ADAM tells you which criteria are still failing and where to fix them so the next phase starts on stronger footing.",
    body: `
# Phase Gate Blockers

If you cannot advance to the next phase, a phase gate is blocking you. This is intentional - ADAM protects downstream work from being built on an incomplete foundation.

## How to see what is blocking you

1. Click the **Advance Phase** button
2. If the gate is blocked, ADAM shows a list of failing exit criteria
...
You do not need to manually maintain the Twin - ADAM builds it from your program data.
...
- Use the **Controls** panel (bottom-left) to fit the graph to screen or lock layout
  ...
  },
  {
    ...
    summary: "Check the sync signal first. ADAM distinguishes between safely saved local work and a true server-side save problem.",
    body: `
...
2. Try refreshing the page - ADAM will attempt to sync on reload
...
ADAM queues AI requests to prevent overload. If you have triggered multiple AI operations at once, they process in sequence. Wait a moment and the response will appear.
