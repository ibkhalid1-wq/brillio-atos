...
PROVISIONAL DETERMINISM: the standard BOUNDS a provisional ontology as well as deepening it. With no discovery conversations on record, every entity must come from exactly one of two sources — (a) a noun the mandate EXPLICITLY names, or (b) a backbone concept of the steered reference model that the mandate's core process DIRECTLY involves (the actor's organisation, the thing requested/exchanged, the named outcome measure). Nothing else qualifies: a concept that is merely plausible for the domain (an activity log, a communication record, a supporting document type the mandate never mentions) is a GAP QUESTION, never an entity. When torn between entity and gap, choose gap. The closed set cuts BOTH ways — it is also a floor: every noun the mandate EXPLICITLY names as an actor, a process stage, a named system or platform, or an outcome measure MUST appear as an entity; dropping a mandate-named noun (the named CRM, a named stage) is exactly as wrong as inventing one. Two runs over the same mandate and industry must produce the SAME entity list — treat the closed set (mandate nouns ∪ directly-involved standard backbone) as the whole universe until interviews widen it.
...
STANDARD-GROUNDED RELATIONS: entities alone do not ground an ontology — relations carry the model's shape, and on a PROVISIONAL draft they must be exactly as grounded and reproducible as the entities. Every provisional relation comes from exactly one of three sources — nothing else qualifies:
...
(c) a named outcome measure "measures" the process or thing it qualifies, and a named system "supports" each stage or process the mandate says it improves.
