...
When the EVIDENCE mentions a role, team or function that is not yet in the cast — an interviewee says "legal reviews it", "the bench team assigns", "we hand it to compliance" — ADD it as a persona (and, if it needs its own voice, an interview placeholder): a role named in testimony is a cast member, not a passing detail.

Derive the FULL CAST from the industry and the objective, not just from who the evidence happens to name. Walk the objective's value chain in THIS industry end to end and enumerate every role that owns, executes, approves, funds, oversees, or is served by the workflow — explicitly including the oversight and adjacent functions this industry requires (e.g. pharma/clinical: regulatory affairs, IRB/ethics, patient advocacy, data management, privacy; banking: credit risk, compliance, audit; retail: supply chain, merchandising). Evidence names the KNOWN voices; the industry names the REQUIRED ones — a cast missing the industry's standard oversight roles is incomplete: emit each as a role placeholder ("Regulatory Affairs Lead — TBC") and list the domain under "gaps".
...
PROVISIONAL DETERMINISM: the standard BOUNDS a provisional ontology as well as deepening it. With no discovery conversations on record, every entity must come from exactly one of two sources — (a) a noun the mandate EXPLICITLY names, or (b) a backbone concept of the steered reference model that the mandate's core process DIRECTLY involves (the actor's organisation, the thing requested/exchanged, the named outcome measure). Nothing else qualifies: a concept that is merely plausible for the domain (an activity log, a communication record, a supporting document type the mandate never mentions) is a GAP QUESTION, never an entity. When torn between entity and gap, choose gap. Two runs over the same mandate and industry must produce the SAME entity list — treat the closed set (mandate nouns ∪ directly-involved standard backbone) as the whole universe until interviews widen it.

MANDATE-VERBATIM NAMING: when the mandate or any evidence names a concept, the entity name IS that wording — title-cased, otherwise verbatim. "increasing enrollment conversion" names the entity "Enrollment Conversion", never "Enrollment Conversion Metric" or "Enrollment Conversion Rate"; "time to enrollment" names "Time to Enrollment". Never append a classifier suffix (Metric, Rate, Status, Outcome, Record) the evidence itself doesn't use, and never substitute a synonym for a noun the evidence supplies. Only a concept the evidence does NOT name takes its noun from the steered standard (Patient, Organization…). Naming is part of determinism — two runs must pick the same nouns, and verbatim evidence wording is the tiebreak.
...
Return ONLY valid JSON:
{
  ...
  "theme": { "brandHue": "the primary brand colour as a hex — chosen for the client's industry/personality, not a default blue", "accent": "a secondary accent hex", "neutral": "the neutral/grey base hex, biased slightly toward the brand hue", "surface": "page background hex (light unless the personality says otherwise)", "good": "#hex", "warn": "#hex", "critical": "#hex", "fontStack": "a system font stack expressing the personality", "typeScaleRatio": 1.2, "spacingBase": 8, "radius": "corner radius in px (e.g. 8 or 12)", "density": "spacious|balanced|dense", "personalityNote": "one line on the feel these tokens create" },
...
// ─── Deterministic provisional ontology (extract → compile → judge) ──────────
// With no discovery conversation on record, the domain ontology is drafted from
// the Frame mandate alone — and free-form authorship of that draft proved
// non-deterministic run-to-run: same mandate, different entity sets, nouns and
// relation phrasings. This path restructures the provisional draft so the model
// only CLASSIFIES over closed choices and pure code does the authoring:
//   1. EXTRACT — verbatim spans from the mandate (actors, things, systems,
//      measures, stages, events, unknowns). Span identification, not composition.
//   2. JUDGE — closed-choice calls: which backbone concepts of the steered
//      standard the mandate directly involves but never names; standard URIs
//      for the extracted entities; relations restricted to a fixed verb menu
//      over the fixed entity list.
//   3. COMPILE — pure code: canonical naming (mandate wording verbatim,
//      title-cased), deduping, fixed ordering, templated evidence/gaps/summary.
//      Byte-stable given the same extract + judge answers.
// The moment any Listen conversation lands, generation returns to full LLM
// synthesis — evidence anchors it there, and synthesis is what evidence needs.

const PROVISIONAL_SMALL_WORDS = new Set([
  "a", "an", "and", "as", "at", "by", "for", "in", "of", "on", "or", "per", "the", "to", "via", "with",
]);

/** Codepoint comparator — locale-independent so canonical ordering never drifts. */
function provisionalCompare(a: string, b: string): number {
  return a < b ? -1 : a > b ? 1 : 0;
}

/** Mandate wording, title-cased per word. Words already carrying a capital
 * anywhere (AI, CRM, iPhone) are kept verbatim so acronyms survive; small
 * words stay lowercase after the first word. */
function provisionalTitleCase(raw: string): string {
  return raw.trim().replace(/\s+/g, " ").split(" ").map((word, index) => {
    if (/[A-Z]/.test(word)) return word;
    const lower = word.toLowerCase();
    if (index > 0 && PROVISIONAL_SMALL_WORDS.has(lower)) return lower;
    return lower.replace(/(^|[-/])(\p{L})/gu, (_m, sep, ch) => `${sep}${ch.toUpperCase()}`);
  }).join(" ");
}

/** Dedupe key: case / punctuation / plural / small-word insensitive, so
 * "Time-to-Enrollment", "time to enrollment" and "Enrollment Times" collide. */
function provisionalEntityKey(name: string): string {
  return name.toLowerCase().replace(/[^\p{L}\p{N}]+/gu, " ").trim().split(/\s+/)
    .map((w) => w.length > 3 && w.endsWith("s") && !w.endsWith("ss") ? w.slice(0, -1) : w)
    .filter((w) => w && !PROVISIONAL_SMALL_WORDS.has(w))
    .join(" ");
}

/** True when any Listen conversation is on the record — the boundary between
 * the deterministic provisional draft and evidence-anchored LLM synthesis.
 * Mirrors the PROVISIONAL SEED condition in the domain-ontology prompt: any
 * non-private listen string field long enough to be a captured conversation. */
function listenDiscoveryOnRecord(inner: Record<string, unknown>): boolean {
  const phaseInputs = normalizeProgramData(inner.phaseInputs as JsonValue | null);
...
function buildProvisionalMandate(
  inner: Record<string, unknown>,
  programRow: Record<string, unknown>,
): ProvisionalMandate | null {
  const phaseInputs = normalizeProgramData(inner.phaseInputs as JsonValue | null);
  const frame = normalizeProgramData(phaseInputs.frame as JsonValue | null);
  const projectMeta = normalizeProgramData(inner.projectMeta as JsonValue | null);
  const str = (v: unknown) => typeof v === "string" ? v.trim() : "";
  const mandate = [str(frame.sponsorConversation), str(frame.objective), str(frame.businessObjective)]
    .filter(Boolean).join("\n\n");
  if (mandate.length < 40) return null;
  return {
    mandate: mandate.slice(0, 8000),
...
const PROVISIONAL_EXTRACT_SYSTEM = `You are the ATOS mandate span extractor. You identify VERBATIM spans in a sponsor mandate — you never invent, rephrase, generalise or add concepts. Every "name" you return must appear word-for-word in the mandate text (case-insensitive; you may drop a leading article).
Return ONLY valid JSON:
{
  "actors": [ { "name": "verbatim span — a person, role or organisation that acts", "quote": "the mandate fragment it appears in" } ],
  "things": [ { "name": "verbatim span — a business object or process the mandate is about", "quote": "..." } ],
  "systems": [ { "name": "verbatim span — a named system, platform or tool", "quote": "..." } ],
  "measures": [ { "name": "verbatim span — a named outcome measure or KPI", "quote": "..." } ],
  "stages": [ { "name": "verbatim span — a step of the named process funnel", "quote": "...", "order": 1 } ],
  "events": [ { "name": "business event the mandate implies, named strictly from mandate words", "triggers": "what causes it, from the mandate", "produces": "what it changes, from the mandate" } ]
}
Rules: nouns only; at most 12 spans in total across actors/things/systems/measures/stages; no duplicates across categories (classify each span once, in its best category); the signing sponsor's own name and their job title are NEVER spans — they sign the mandate, they are not part of the domain; baseline phrases in parentheses are context for measures, not separate spans; "order" is a stage's 1-based position in the mandate's stated sequence; empty arrays when nothing qualifies; max 5 events.`;

const PROVISIONAL_JUDGE_SYSTEM = `You are the ATOS ontology adjudicator. You answer ONE closed question: which of the listed BACKBONE concepts of the industry reference model does this mandate's core process DIRECTLY involve? You never rename a concept, never add anything beyond the given list, never write prose.
Return ONLY valid JSON:
{ "involved": [ "backbone concept name EXACTLY as listed" ] }
DIRECTLY involved means the mandate's process could not run without it — the organisation an actor belongs to, the thing being requested or exchanged, the record the process produces. A concept that is merely plausible for the industry is NOT involved. When in doubt, leave it out. Max 4.`;

type ProvisionalCandidate = {
  name: string;
  kind: "actor" | "thing" | "stage" | "system" | "measure";
  quote: string;
};

/** Majority-vote merge over N independent extractions. A span survives only
 * when a MAJORITY of runs found it (any category), so one flaky sample can
 * neither drop the process spans nor smuggle in a spurious one. Its category
 * is the per-field vote winner; its representative fields are the field-wise
 * lexicographic minimum across runs — canonical regardless of run order. */
function mergeProvisionalExtracts(extracts: Array<Record<string, unknown>>): Record<string, unknown> {
  const ENTITY_FIELDS = ["actors", "things", "systems", "measures", "stages"] as const;
  const majority = Math.floor(extracts.length / 2) + 1;
  const canonicalRow = (rows: Array<Record<string, unknown>>): Record<string, unknown> => {
    const rep: Record<string, unknown> = {};
    for (const field of new Set(rows.flatMap((r) => Object.keys(r)))) {
      const vals = rows.map((r) => r[field]).filter((v) => v !== undefined);
      const strs = vals.filter((v): v is string => typeof v === "string").sort(provisionalCompare);
      const nums = vals.filter((v): v is number => typeof v === "number").sort((x, y) => x - y);
      rep[field] = strs.length ? strs[0] : nums.length ? nums[0] : vals[0];
    }
    return rep;
  };
  // Entity spans: vote by normalised key ACROSS categories (a run classifying
  // "patient enrollment" as a stage instead of a thing still counts), then the
  // winning category takes the span.
  type Bucket = { total: number; byField: Map<string, number>; rows: Array<Record<string, unknown>> };
  const buckets = new Map<string, Bucket>();
  for (const extract of extracts) {
    const seenThisRun = new Set<string>();
    for (const field of ENTITY_FIELDS) {
      const rows = Array.isArray(extract[field]) ? (extract[field] as unknown[]).filter(isRecord) : [];
      for (const row of rows) {
        const key = provisionalEntityKey(typeof row.name === "string" ? row.name : "");
        if (!key || seenThisRun.has(key)) continue;
        seenThisRun.add(key);
        const bucket = buckets.get(key) ?? { total: 0, byField: new Map(), rows: [] };
        bucket.total++;
        bucket.byField.set(field, (bucket.byField.get(field) ?? 0) + 1);
        bucket.rows.push(row);
        buckets.set(key, bucket);
      }
    }
  }
  const merged: Record<string, unknown> = { actors: [], things: [], systems: [], measures: [], stages: [] };
  for (const bucket of buckets.values()) {
    if (bucket.total < majority) continue;
    let winner: string = ENTITY_FIELDS[0];
    let winnerVotes = -1;
    for (const field of ENTITY_FIELDS) {
      const votes = bucket.byField.get(field) ?? 0;
      if (votes > winnerVotes) { winner = field; winnerVotes = votes; }
    }
    (merged[winner] as Array<Record<string, unknown>>).push(canonicalRow(bucket.rows));
  }
  // Events: same majority vote, keyed on the event name.
  const eventBuckets = new Map<string, Array<Record<string, unknown>>>();
  for (const extract of extracts) {
    const seenThisRun = new Set<string>();
    for (const row of (Array.isArray(extract.events) ? extract.events as unknown[] : []).filter(isRecord)) {
      const key = provisionalEntityKey(typeof row.name === "string" ? row.name : "");
      if (!key || seenThisRun.has(key)) continue;
      seenThisRun.add(key);
      eventBuckets.set(key, [...(eventBuckets.get(key) ?? []), row]);
    }
  }
  merged.events = [...eventBuckets.values()].filter((rows) => rows.length >= majority).map(canonicalRow);
  return merged;
}

function compileProvisionalCandidates(extract: Record<string, unknown>, sponsor: string): ProvisionalCandidate[] {
  const kinds: Array<[ProvisionalCandidate["kind"], string]> = [
    ["actor", "actors"], ["thing", "things"], ["stage", "stages"], ["system", "systems"], ["measure", "measures"],
  ];
  // The signing sponsor and their title are the mandate's AUTHOR, not part of
  // the domain — never entities. Belt-and-braces with the extractor rule.
  const seen = new Set<string>([provisionalEntityKey(sponsor), "sponsor", "executive sponsor"].filter(Boolean));
  const out: ProvisionalCandidate[] = [];
  for (const [kind, field] of kinds) {
    const rows = Array.isArray(extract[field]) ? extract[field] as unknown[] : [];
    const named = rows.filter(isRecord)
      .map((row) => ({
        name: typeof row.name === "string" ? row.name.trim() : "",
        quote: typeof row.quote === "string" ? row.quote.trim().slice(0, 220) : "",
      }))
      .filter((row) => row.name.length > 1 && row.name.length < 80);
    // Canonical order INSIDE each kind — extraction order is not stable, the
    // sorted order is.
    named.sort((a, b) => provisionalCompare(a.name.toLowerCase(), b.name.toLowerCase()));
    for (const row of named) {
      const key = provisionalEntityKey(row.name);
      if (!key || seen.has(key)) continue;
      seen.add(key);
      out.push({ name: provisionalTitleCase(row.name), kind, quote: row.quote });
    }
  }
  return out.slice(0, 14);
}

const PROVISIONAL_KIND_DEFINITION: Record<ProvisionalCandidate["kind"] | "backbone", string> = {
  actor: "Acts in the mandated process",
  thing: "What the mandated process is about",
  stage: "A stage of the mandated process",
  system: "A system the mandate names",
  measure: "An outcome measure the mandate names",
  backbone: "Backbone concept of the industry reference model",
};

// ─── Standard backbone packs ──────────────────────────────────────────────────
// The steered standards as DATA, not prompt prose. Each pack distils a
// reference model's backbone into named entities (with alias lists for
// deterministic code-side matching against mandate spans) and canonical
// relations between them. The judge votes yes/no over this CLOSED list —
// never recalls the standard from training — so a backbone concept can only
// enter the ontology under its pack name and pack URI. Entities whose
// standard publishes no public linked-data URI (FIBO deep paths, CIM, some
// EBUCore classes) carry uri:"" and align by name only, exactly as the
// steering prose already promises for ACORD/SID.
...
/** Alias index over the resolved packs: normalised alias/name key → the pack
 * entity that owns it. First pack wins (primary vocabulary outranks fallback). */
function provisionalAliasIndex(packs: ProvisionalPack[]): Map<string, { entity: ProvisionalPackEntity; vocabulary: string }> {
  const index = new Map<string, { entity: ProvisionalPackEntity; vocabulary: string }>();
  for (const pack of packs) {
    for (const entity of pack.entities) {
      for (const alias of [entity.name, ...entity.aliases]) {
        const key = provisionalEntityKey(alias);
        if (key && !index.has(key)) index.set(key, { entity, vocabulary: pack.vocabulary });
      }
    }
  }
  return index;
...
/** Match a mandate span to a pack entity. Whole-key match for every kind; a
 * SYSTEM span additionally matches on any single word ("AI-powered CRM" hits
 * the "crm" alias → SoftwareApplication). Only systems get word-level matching
 * — a generic noun inside a compound thing ("patient" in "Patient Engagement")
 * must NOT count as covering the pack concept. */
function provisionalPackMatch(
  aliasIndex: Map<string, { entity: ProvisionalPackEntity; vocabulary: string }>,
  name: string,
  kind: ProvisionalCandidate["kind"],
): { entity: ProvisionalPackEntity; vocabulary: string } | undefined {
  const key = provisionalEntityKey(name);
  const direct = aliasIndex.get(key);
  if (direct) return direct;
  if (kind === "system") {
    for (const word of key.split(" ")) {
      const byWord = aliasIndex.get(word);
...
function compileProvisionalOntologyDoc(
  m: ProvisionalMandate,
  candidates: ProvisionalCandidate[],
  extract: Record<string, unknown>,
  judge: Record<string, unknown>,
  packs: ProvisionalPack[],
): Record<string, unknown> {
  type Row = { name: string; kind: ProvisionalCandidate["kind"] | "backbone"; evidence: string; definition: string };
  const byKey = new Map<string, Row>();
  const order: string[] = [];
  const aliasIndex = provisionalAliasIndex(packs);
  // Which pack entity each doc entity stands for (pack name → display name in
  // the doc) — drives pack relations and code-side standard alignment.
  const packPresence = new Map<string, string>();
  const alignmentRows: Array<{ entity: string; standard: string; vocabulary: string; relation: string; confidence: number }> = [];
  for (const c of candidates) {
    const key = provisionalEntityKey(c.name);
    if (byKey.has(key)) continue;
    byKey.set(key, {
      name: c.name,
      kind: c.kind,
      evidence: c.quote ? `"${c.quote}" — sponsor mandate, to confirm` : "from the sponsor mandate — to confirm",
      definition: `${PROVISIONAL_KIND_DEFINITION[c.kind]} — to be confirmed in interviews.`,
    });
    order.push(key);
    // Code-side alignment: the span matches a pack entity's name or alias.
    // MANDATE-VERBATIM NAMING holds — the doc keeps the mandate's wording, the
    // pack contributes the URI.
    const match = provisionalPackMatch(aliasIndex, c.name, c.kind);
    if (match) {
      if (!packPresence.has(match.entity.name)) packPresence.set(match.entity.name, c.name);
      if (match.entity.uri) {
        alignmentRows.push({ entity: c.name, standard: match.entity.uri, vocabulary: match.vocabulary, relation: "skos:closeMatch", confidence: 0.8 });
      }
    }
  }
  // Backbone additions: the judge's yes-votes over the CLOSED pack list, minus
  // anything a mandate span already covers. Pack name, pack URI, pack
  // definition — nothing model-authored.
  const votes = new Set(
    (Array.isArray(judge.involved) ? judge.involved as unknown[] : [])
      .filter((v): v is string => typeof v === "string")
      .map((v) => provisionalEntityKey(v)),
  );
  const backbone: Array<{ entity: ProvisionalPackEntity; vocabulary: string }> = [];
  const seenPackNames = new Set<string>();
  for (const pack of packs) {
    for (const entity of pack.entities) {
      if (seenPackNames.has(entity.name)) continue;
      seenPackNames.add(entity.name);
      if (!votes.has(provisionalEntityKey(entity.name))) continue;
      if (packPresence.has(entity.name)) continue;
      if (byKey.has(provisionalEntityKey(entity.name))) continue;
      backbone.push({ entity, vocabulary: pack.vocabulary });
    }
  }
  backbone.sort((a, b) => provisionalCompare(a.entity.name, b.entity.name));
  for (const { entity, vocabulary } of backbone.slice(0, 4)) {
    const key = provisionalEntityKey(entity.name);
    byKey.set(key, {
      name: entity.name,
      kind: "backbone",
      evidence: `implied by ${vocabulary} — to confirm`,
      definition: `${entity.definition} — to be confirmed in interviews.`,
    });
    order.push(key);
    packPresence.set(entity.name, entity.name);
    if (entity.uri) {
      alignmentRows.push({ entity: entity.name, standard: entity.uri, vocabulary, relation: "skos:exactMatch", confidence: 0.9 });
    }
  }
  const entities = order.map((key) => {
    ...
    return {
      ...
      systemOfRecord: e.kind === "system" ? e.name : null,
      aliases: [] as string[],
      evidence: e.evidence,
    };
  });
  // Relations — fully deterministic: the packs' canonical relations whose
  // endpoints are both present, plus the mandate's stated stage sequence
  // chained in order. Cardinality is always "unknown" on a provisional draft:
  // cardinality is exactly what interviews confirm.
  const relKeys = new Set<string>();
  const relations: Array<{ from: string; relation: string; to: string; cardinality: string }> = [];
  for (const pack of packs) {
    for (const rel of pack.relations) {
      const from = packPresence.get(rel.from);
      const to = packPresence.get(rel.to);
      if (!from || !to || from === to) continue;
      const dupe = `${from}|${to}`;
      if (relKeys.has(dupe)) continue;
      relKeys.add(dupe);
      relations.push({ from, relation: rel.verb, to, cardinality: "unknown" });
    }
  }
  const stageNames: string[] = [];
  const stageSeen = new Set<string>();
  const stageRows = (Array.isArray(extract.stages) ? extract.stages as unknown[] : [])
    .filter(isRecord)
    .map((row) => ({
      name: typeof row.name === "string" ? row.name.trim() : "",
    ...
    .filter((row) => row.name)
    .sort((a, b) => a.order - b.order || provisionalCompare(a.name.toLowerCase(), b.name.toLowerCase()));
  for (const row of stageRows) {
    const name = byKey.get(provisionalEntityKey(row.name))?.name;
    if (!name || stageSeen.has(name)) continue;
    stageSeen.add(name);
    stageNames.push(name);
  }
  for (let i = 0; i + 1 < stageNames.length; i++) {
    const dupe = `${stageNames[i]}|${stageNames[i + 1]}`;
    if (relKeys.has(dupe)) continue;
    relKeys.add(dupe);
    relations.push({ from: stageNames[i], relation: "leads to", to: stageNames[i + 1], cardinality: "unknown" });
  }
  relations.sort((a, b) => provisionalCompare(`${a.from}|${a.to}`, `${b.from}|${b.to}`));
  // Events from the extraction — mandate-worded, canonicalised.
  const eventKeys = new Set<string>();
  const events = (Array.isArray(extract.events) ? extract.events as unknown[] : [])
    .filter(isRecord)
    .map((row) => ({
      name: typeof row.name === "string" ? provisionalTitleCase(row.name.trim()) : "",
      triggers: typeof row.triggers === "string" ? row.triggers.trim().slice(0, 200) : "",
      produces: typeof row.produces === "string" ? row.produces.trim().slice(0, 200) : "",
    }))
    .filter((row) => {
      if (row.name.length < 3 || row.name.length > 80) return false;
      const key = provisionalEntityKey(row.name);
      if (!key || eventKeys.has(key)) return false;
      eventKeys.add(key);
      return true;
    })
    .sort((a, b) => provisionalCompare(a.name, b.name))
    .slice(0, 5);
  // Standard alignment — entirely code-derived (alias matches + voted backbone),
  // one row per entity, canonically sorted. No model-authored URI ever lands.
  const alignedEntities = new Set<string>();
  const standardAlignment: Array<{ entity: string; standard: string; vocabulary: string; relation: string; confidence: number }> = [];
  alignmentRows.sort((a, b) => provisionalCompare(a.entity, b.entity));
  for (const row of alignmentRows) {
    if (alignedEntities.has(row.entity)) continue;
    alignedEntities.add(row.entity);
    standardAlignment.push(row);
  }
  // Gaps: templated from what the compile itself established — measures need
  // baselines, systems need ownership, voted backbone concepts need
  // confirmation. Code-derived, so two runs ask the same questions.
  const ask = m.sponsor ? `Ask ${m.sponsor}: ` : "Ask the sponsor: ";
  const gaps: string[] = [];
  for (const key of order) {
    const e = byKey.get(key)!;
    if (e.kind === "measure") gaps.push(`${ask}What is the current baseline for ${e.name}, and where is it measured today?`);
    if (e.kind === "system") gaps.push(`${ask}Which records does ${e.name} own today, and who works in it?`);
    if (e.kind === "backbone") gaps.push(`${ask}How does ${e.name} feature in the process today, and who owns it?`);
  }
  gaps.sort(provisionalCompare);
  gaps.splice(6);
  return {
    title: `Domain Ontology — ${m.programName}`,
    entities,
    relations,
    events,
    standardAlignment,
    ...
    gaps,
    summary: `Provisional ontology — compiled from the sponsor mandate with the steered standard backbone; ${entities.length} entities to confirm, correct and extend in interviews.`,
    confidence: 0.4,
  };
}

/** The provisional generation runner. Returns a streamClaudeText-shaped result
 * (so the shared repair / cost-ledger / persist path downstream is untouched),
 * or null to fall through to normal synthesis: evidence already on record, no
 * usable mandate, an unusable extraction, or any error in the new path. */
async function runProvisionalOntologyGeneration(
  inner: Record<string, unknown>,
  programRow: Record<string, unknown>,
): Promise<Awaited<ReturnType<typeof streamClaudeText>> | null> {
  try {
    if (listenDiscoveryOnRecord(inner)) return null;
    const m = buildProvisionalMandate(inner, programRow);
    if (!m) return null;
    // THREE independent extractions, majority-merged: span identification at
    // temperature 0 is stable but not perfectly so, and one sparse sample must
    // not change the compiled ontology. ~1.6k tokens each — still far cheaper
    // than one free-authored full document.
    const extractResults = await Promise.all([0, 1, 2].map(() =>
      streamClaudeText({
        system: PROVISIONAL_EXTRACT_SYSTEM,
        messages: [{ role: "user", content: JSON.stringify({ mandate: m.mandate }) }],
        maxTokens: 1600,
        temperature: 0,
      })));
    const parsedExtracts = extractResults
      .map((r) => extractAgentJson(r.text))
      .filter(isRecord);
    if (!parsedExtracts.length) return null;
    const extract = mergeProvisionalExtracts(parsedExtracts);
    const candidates = compileProvisionalCandidates(extract, m.sponsor);
    if (!candidates.length) return null;
    // The judge's ballot: every pack entity not already covered by a mandate
    // span, in the packs' own stable order. A closed list — the model can only
    // vote names on it, never recall the standard from training.
    const packs = resolveProvisionalPacks(ontologyVocabularySteering(m.industry, m.segment));
    const aliasIndex = provisionalAliasIndex(packs);
    const covered = new Set(
      candidates.map((c) => provisionalPackMatch(aliasIndex, c.name, c.kind)?.entity.name).filter(Boolean),
    );
    const ballotNames = new Set<string>();
    const ballot: Array<{ name: string; vocabulary: string; definition: string }> = [];
    for (const pack of packs) {
      for (const entity of pack.entities) {
        if (ballotNames.has(entity.name) || covered.has(entity.name)) continue;
        ballotNames.add(entity.name);
        ballot.push({ name: entity.name, vocabulary: pack.vocabulary, definition: entity.definition });
      }
    }
    const judgeResult = await streamClaudeText({
      system: PROVISIONAL_JUDGE_SYSTEM,
      messages: [{
        role: "user",
        content: JSON.stringify({ mandate: m.mandate, backbone: ballot }),
      }],
      maxTokens: 800,
      temperature: 0,
    });
    const judgeParsed = extractAgentJson(judgeResult.text);
    const judge = isRecord(judgeParsed) ? judgeParsed : {};
    const doc = compileProvisionalOntologyDoc(m, candidates, extract, judge, packs);
    return {
      ...judgeResult,
      text: JSON.stringify(doc),
      inputTokens: extractResults.reduce((sum, r) => sum + r.inputTokens, judgeResult.inputTokens),
      outputTokens: extractResults.reduce((sum, r) => sum + r.outputTokens, judgeResult.outputTokens),
      cachedInputTokens: extractResults.reduce((sum, r) => sum + (r.cachedInputTokens ?? 0), judgeResult.cachedInputTokens ?? 0),
    };
  } catch {
    // Never let the deterministic path take a run down — synthesis still works.
    return null;
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Deterministic provisional ontology: with no Listen conversation on record
    // the domain ontology is COMPILED (extract → compile → judge — see the
    // provisional-ontology section) rather than free-authored, so identical
    // mandates yield identical drafts. Null (evidence on record, no mandate, or
    // an unusable extraction) falls through to normal synthesis.
    let claudeResult = request.agentId === "domain-ontology"
      ? await runProvisionalOntologyGeneration(getInnerProgramData(contextProgramData), programRow as Record<string, unknown>)
      : null;
    if (!claudeResult) {
